...
public static class RegenUtil
{
    ...
    private static string MutateOT(string OT, LanguageID? lang, GameVersion game)
    {
        ...
        var max = Legal.GetMaxLengthOT(game.Context.Generation, (LanguageID)lang);
        OT = OT[..Math.Min(OT.Length, max)];
        if (GameVersion.GG.Contains(game) || game.Context.Generation >= 8) // switch keyboard only has latin characters, --don't mutate
            return OT;
    ...
    public static string MutateNickname(string nick, LanguageID? lang, GameVersion game)
    {
        ...
        if (game.Context.Generation >= 8 || lang == null)
            return nick;
