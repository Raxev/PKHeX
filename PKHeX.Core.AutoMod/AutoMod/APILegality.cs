...
public static class APILegality
{
    ...
    internal static GameVersion[] FilteredGameList(PKM template, GameVersion destVer, bool batchEdit, IBattleTemplate set, bool nativeOnly = false)
    {
        ...
        var gamelist = !nativeOnly ? [.. versionlist.OrderByDescending(c => c.Context.Generation)] : GetPairedVersions(destVer, versionlist);
        if (PrioritizeGame)
        ...
        if (template.AbilityNumber == 4 && destVer.Context.Generation < 8)
            gamelist = [.. gamelist.Where(z => z.Context.Generation is not 3 and not 4)];
    ...
    private static bool IsEncounterValid(IBattleTemplate set, IEncounterTemplate enc, AbilityRequest abilityreq, GameVersion destVer)
    {
        ...
        var gen = enc.Generation;
        if (abilityreq == AbilityRequest.Hidden && gen is 3 or 4 && destVer.Context.Generation < 8)
            return false;
